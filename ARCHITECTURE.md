...
4. Extend the iPhone Live Activity push path (implemented behind `CAMP_PUSH` + `CAMP_APNS_*`): broadcast channels (iOS 18+) would let a push-started activity take updates without waiting for the app to register its update token once, and would scale to multiple phones.
