...
`MacLunchCalendar` owns EventKit access and publishes only calendar choices, anonymous free intervals and current availability. `CampSettingsStore` configures it from successfully saved meal preferences and office timezone and exposes it to directly observing calendar views. Calendar selection persists separately in UserDefaults; access is always checked against the OS. `CampCalendarView` replaces fixture calendar switches and connection previews. The service never uploads event content and writes only to the dedicated **camp** calendar it creates (`campCalendar()`, id in UserDefaults): `addOrderEvent` writes a one-off block or a weekly-recurring one (backend weekdays 0 = Monday mapped onto `EKWeekday`), `removeOrderEvent` deletes it and its future occurrences. The reader skips that calendar so camp's own blocks never count as busy. `CampSettingsStore.syncOrderEvents` keeps one block per joined group (keyed by group id in UserDefaults); `addSchedule` writes the recurring block and stores its id on the backend schedule. See `docs/CALENDAR.md`.


## Shared group flow

`CampSettingsStore.lunchGroups` is `GET /v1/groups` for the saved office and today (`backend/src/camp/groups.py`, `groups` table). Every group carries a `category` (`coffee` | `meal`). Creation is `POST /v1/groups` at any catalog place serving that category; joining is `POST /v1/groups/{id}/join` with any item from the place's full menu (`GET /v1/restaurants/{id}/menu`, which also returns the public rating and the user's recommender-ranked top picks), which the backend treats as one order per person per category per day (it leaves other groups in the same category only) and which upserts a confirmed `Order` for the member; leaving is a `DELETE` that cancels it. Standing orders (`/v1/schedules`, `schedules` table) are materialised by `GroupService.today` for the requesting user: each active schedule due that weekday puts the user in a matching group at that place and time, creating one when needed. Restaurant rankings use a review-count-shrunk rating so a 5.0 from six reviews does not outrank a 4.6 from thousands; seeded meal groups skip cafés. Participant counts, delivery-fee shares, per-option prices, `peopleOrdering` and `totalSavingsCents` are computed server-side from real membership. `DemoLunchGroup` keeps its historical name as the Swift shape of a group row; `people` excludes the current user so the optimistic local join still renders correctly.
